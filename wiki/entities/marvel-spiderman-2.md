---
title: "마블 스파이더맨 2 (Marvel's Spider-Man 2)"
type: entity
sources: ["[[gdc24-spiderman2-open-world]]", "[[gdc2019-spiderman-animation]]"]
related: ["[[insomniac-games|인섬니악 게임즈]]", "[[open-world-design|오픈월드 설계]]", "[[player-guidance-design|플레이어 유도 설계]]", "[[cinematic-production|시네마틱 제작]]"]
created: 2026-04-20
updated: 2026-06-01
confidence: high
relations:
  developedBy: [insomniac-games]
  publishedBy: [sony-interactive-entertainment]
  platform: [ps5]
---

인섬니악 게임즈(Insomniac Games) 개발, 소니 퍼스트 파티.

| 항목 | 내용 |
|------|------|
| 개발사 | [[insomniac-games]] |
| 장르 | 오픈월드 액션 어드벤처 |
| 출시 | 2023년 10월 |
| 특기 | 맵 전면 공개 + 탐험 유도 설계 |

## 오픈월드 설계 진화

- **전작(1편)**: 타워 해금 방식 → 플레이어들이 타워 클리어에만 집중, 중간 콘텐츠 무시
- **2편 첫 시도**: 전체 맵 공개 → 선택지 과부하로 네비게이션만 찍어 이동
- **최종 해결**: 특정 구간 빠른 이동 잠금 + 탐험 먼저 유도 → 발견·목적·미지의 조합으로 탐험 동기 설계

→ 상세: [[gdc24-spiderman2-open-world]]
