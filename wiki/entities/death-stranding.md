---
title: "데스 스트랜딩 (Death Stranding)"
type: entity
sources: ["[[gdc2021-death-stranding-ai]]"]
related: ["[[kojima-productions|코지마 프로덕션즈]]", "[[ai-navigation|AI 내비게이션]]", "[[open-world-design|오픈월드 설계]]"]
created: 2026-04-20
updated: 2026-06-01
confidence: high
relations:
  developedBy: [kojima-productions]
  publishedBy: [sony-interactive-entertainment]
  platform: [ps4, ps5, pc]
---

코지마 히데오 감독, 코지마 프로덕션즈 개발. 독특한 오픈월드 배달 게임.

| 항목 | 내용 |
|------|------|
| 개발사 | [[kojima-productions]] |
| 장르 | 오픈월드 액션 |
| 출시 | 2019년 |
| 특기 | 복잡 지형 AI 내비게이션, 사회적 연결 메커니즘 |

## 게임 디자인 특징

- **AI 내비게이션**: 험준한 산악·강 지형에서의 경로탐색 문제 → [[ai-navigation|AI 내비게이션]] 참조
- **비폭력 오픈월드**: 전투보다 운반과 연결이 핵심
- GDC 강연 참조: [[gdc2021-death-stranding-ai]]
