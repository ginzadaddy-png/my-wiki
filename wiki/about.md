...
## 현재 통계·규모 (2026-08-03 기준)

| 항목 | 수치 |
|---|---|
| 누적 INGEST 건수 | 103건 |
| 소스 요약 (`wiki/sources/`) | 166개 |
| 개념 페이지 (`wiki/concepts/`) | 70개 |
| 엔티티 페이지 (스튜디오·게임·플랫폼·인물, `wiki/entities/`) | 100개 |
...
총 약 367개 md 페이지가 wikilink로 연결되어 있고, 모두 frontmatter(`type`, `sources`, `related`, `confidence` 등) 기반으로 구조화되어 있어 graph 추론·검색에 활용되고 있습니다. 2026년 6월부터는 이 위키 전체를 검색·추론하는 **챗봇 에이전트**가 별도 트랙으로 가동 중입니다.
