---
title: "Steam 마케팅 전략 — 슬라이드"
type: presentation
sources: ["[[zukowski-next-fest-strategy]]", "[[steam-next-fest-2026-analysis]]", "[[steam-page-optimization-guide-2026]]", "[[game-marketing-strategy-2026]]", "[[airbridge-steam-marketing-playbook]]"]
related: ["[[marketing-strategy|마케팅 전략]]", "[[launch-metrics|흥행 예측 지표]]", "[[indie-business-strategy|인디 비즈니스 전략]]"]
created: 2026-05-11
updated: 2026-05-12
confidence: medium
---

위키의 마케팅·출시 관련 페이지를 종합해 인디·중소 스튜디오용 Steam 출시 전략을 슬라이드로 재구성. 스토어 페이지·태그·위시리스트·데모·Next Fest·세일 정책·아시아 스튜디오가 놓치는 채널까지 한 흐름으로 정리.

<p><strong>전체 화면으로 보기</strong>: <a href="/quartz/presentations/steam-marketing-strategy.html" target="_blank" rel="noopener">슬라이드 열기 ↗</a></p>

<iframe
  src="/quartz/presentations/steam-marketing-strategy.html"
  width="100%"
  height="600"
  style="border: 1px solid var(--lightgray); border-radius: 6px;"
  loading="lazy"
  title="Steam 마케팅 전략 슬라이드">
</iframe>

## 주요 출처

- [[zukowski-next-fest-strategy]] — Steam Next Fest 2026-02 정량 분석
- [[steam-next-fest-2026-analysis]] — 페스트 알고리즘 변화·CL2 vs HMD 비교
- [[steam-page-optimization-guide-2026]] — 스토어 페이지 최적화 + 트레일러 5초 룰
- [[game-marketing-strategy-2026]] — 2026 마케팅 캘린더 + GTA6 윈도우
- [[airbridge-steam-marketing-playbook]] — 중견 스튜디오 유료 마케팅 + 아시아 스튜디오 갭

## 관련 위키 페이지

- [[marketing-strategy|마케팅 전략]] — 본 슬라이드의 베이스 콘셉트 페이지
- [[launch-metrics|흥행 예측 지표]] — 가짜 신호 vs 행동 기반 지표
- [[indie-business-strategy|인디 비즈니스 전략]] — 비즈니스 모델·생존 전략 맥락

## 갱신 메모

- 2026-05-11: 초안 (v1·v2 → v1만 유지)
- 차기 업데이트 시 슬라이드 HTML 갱신 + 본 페이지 updated 날짜만 변경. 위키 콘텐츠(concept·source)에서 truth source 변경 시 슬라이드도 동기화.
