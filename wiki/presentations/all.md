...
- 링크 규칙: 카탈로그·index의 표 안 슬라이드 링크는 raw HTML + **완전 절대 URL** 사용 (`<a href="https://ginzadaddy-png.github.io/quartz/presentations/[슬러그].html" target="_blank" rel="noopener">전체 화면 열기 ↗</a>`) — Quartz가 root-relative path는 변형(`.html` 제거·baseUrl 처리)하지만 외부 절대 URL은 그대로 둠. markdown link도 같은 이유로 회피.
