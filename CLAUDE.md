...
> **자동 실행**: LINT는 **`LLM wiki lint`라는 Claude Code 스케줄 루틴으로 매주 자동 실행**됨 (2026-06-22부터 — 기존 Cowork 주간 스케줄 대체). 루틴은 아래 7단계를 그대로 수행하되 **push 금지·새 페이지 자동 생성 금지·자동 ingest 금지** — 결과를 채팅으로 보고만 하고 실제 반영·push는 사용자 검토 후 진행. 수동으로 "lint"/"점검" 요청 시에도 동일 절차.
...
5. 자동 업데이트 (지정된 섹션·필드만, 나머지는 수동 유지):
   ...
   - **wiki/about.md** (수치·날짜·인라인 통계만):
     - 통계 표의 sources/concepts/entities/comparisons/presentations 카운트
