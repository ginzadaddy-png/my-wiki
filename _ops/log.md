...
- 직전 push는 0fa7927. 누적분 전량 반영, 워킹트리 clean
- 빌드 검사: 통과 (434 파싱 · 916 emit · 41초). 끊긴 wikilink 0 · 완전 고립 0 · 줄바꿈 churn 0(변경 32파일 HEAD 대조)
- deck 신규·수정 없음 -> 단독 배포본(`dist/`) 재생성 불필요
- 챗봇 재색인: 해당 없음 (2026-09-03 delist)
- Quartz repo 변경 없음 -> 별도 push 사이클 불필요
- **changelog 문체 규칙 확정** — 불릿은 개조식 명사 종결(마침표 없음), 존댓말은 *인트로 문단·하단 푸터에만*. 사용자 지적으로 발견했고, 실태는 불릿 57개 중 55개가 이미 개조식이라 다수 쪽으로 3줄만 고쳐 통일(line 17·19 존댓말 -> 개조식, line 27 마침표 제거). 규칙은 스케줄 루틴 파일 5번 changelog 항목에 명문화
...
- 생성(source 1): **sony-ir-1q-fy2026**
  - 핵심 발견: G&NS 매출 +6억엔인데 **환율 기여 +817억엔 → 고정환율 기준 약 -8.7%**. 전사도 고정환율 -1%
  - 영업이익 +37%의 소니 명시 플러스 요인은 **미국 관세 환급·환율 둘뿐**. 마이너스는 서드파티 소프트웨어 감소·하드웨어 유닛 감소·차세대 플랫폼 투자·구조조정
  - PS5 출하 160만(전년 250만, -36%)·누적 9,530만·소프트웨어 6,610만(1st party 600만)·**디지털 82%(전년 83%)**·MAU 1.25억(+2% 6월 최고)·**총 플레이타임 -4%**
  - FY26 가이던스 **매출 4조 5,400억(FY25 대비 -3%)·영업이익 6,600억(+42%)**, 하향요인 퍼스트파티 로드맵 조정
  - 메모리: FY26 필요 물량 확보·하드웨어 수익성 계획 변경 없음 → naavik AI 메모리 쇼크 논지에 부분 반증
- 업데이트(decision 1): 2026-08-03-console-retail-arpu-focus — **반박 ⓕ(채널 전환 분기의 실제 성적표)·ⓖ(디지털 비중 비직진) 신설**, 종합 평가에 "방향은 사실, 성과는 미검증" 추가, 사후 검증에 **FY26 가이던스 달성 여부(2027-05)** 앵커 추가
- 업데이트(concept 2): console-retail-strategy(디지털 정체 ⚠️ + 마진 실적 미검증 단락) · game-market-trends(PS5 -36% 실측 + 메모리 확보 단서)
- 업데이트(source 1): ps-physical-digital-share-2026 — "90%+ 도달" 전망에 **후속 분기가 뒤집었다는 ⚠️ 단서**. 그 전망은 소니 공식 입장이 아니라 기사 추론이었음을 명시
- 업데이트(entity 1): ps5 — 빈 sources·related 채우고 누적 출하·분기 출하·디지털 비중·2028 디스크 중단 기재
- ⚠️ 수치 충돌 해소: 인벤은 PS5 누적 9,370만으로 보도했으나 보충자료·복수 해외 매체는 **9,530만(sell-in)** 으로 일치 → 9,530만 채택하고 페이지에 명시
- 카탈로그: index(소스 169→170) · sources/all(1행)
- 미push — 사용자 검토 후
